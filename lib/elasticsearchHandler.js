...
ElasticsearchStore.prototype.search = function(request, callback) {
  ...
  var params = _.assign(_.cloneDeep(rawParams), self._applyPagination(request), self._applySort(request));
  ...
  async.parallel({
    ...
    totalRows: function(asyncCallback) {
	  debug("count total in search", JSON.stringify(rawParams));
      self._db.count(rawParams, asyncCallback);
    }
